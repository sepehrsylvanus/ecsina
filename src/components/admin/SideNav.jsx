        ...
        <ul className="space-y-2">
          {sideNavLinks.map((item) => {
            return (
              <li
                key={item.id}
                className={`py-3 pl-2 pr-8 cursor-pointer plum-gradient-hover  transition-all ease-linear ${path == item.link ? "plum-gradient" : "plum-gradient-hover"}  `}
              >
      ...
      <dialog id="logout" className="modal">
        <div className="modal-box w-[30%] bg-secondary-background text-title">
          <h1 className="font-bold text-lg flex items-center justify-between mb-14">
            ...
            <button
              onClick={() => document.querySelector("#logout").close()}
          ...
          >
            ...
            <div className="w-full flex gap-3 mt-12">
              ...
              <button
                ...
                className="bg-danger hover:bg-danger-dark w-1/2 p-3 rounded-[20px] text-white"
              >
                خروج از حساب
              </button>
            </div>
